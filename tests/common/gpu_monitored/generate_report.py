...
def _sf(v, default=0.0):
    ...
    Aligned with :func:`tests.common.gpu_monitored.analyze_monitoring._sf`: amd-smi
    emits ``NaN`` when a sensor is transient-out, and ``float("nan")``
    parses successfully. Without this guard NaN leaks into the rendered
    HTML report via ``min``/``max``/``avg`` and shows up as ``NaN`` in
    the summary cells and breaks chart axes.
    """
    try:
        s = str(v).strip()
...
def _parse_cu_rows(rows: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        try:
            timestamp = int(str(row.get("timestamp", "")).strip())
        except (TypeError, ValueError):
            continue
        required = ("gpu", "pid", "cu_occupancy", "vram_mb")
...
def _render_combined_chart(data: Dict[str, List[Dict[str, Any]]], title: str,
    ...
    by_ts = defaultdict(lambda: defaultdict(list))
...
(function() {{
  ...
  svg.addEventListener("mouseleave", function(){{ xline.style.display="none";tip.style.display="none"; }});
    ...
    <div class="chart-card" style="position:relative;">
      ...
      <svg id="{cid}_svg" viewBox="0 0 {w} {h}" width="100%" height="{h}" style="cursor:crosshair;">
        <rect width="{w}" height="{h}" fill="rgba(15,23,42,0.95)" rx="6" />
        {grid}
        {lines}
        {legend}
...
def _render_health_checks(analysis: Dict[str, Any]) -> str:
    ...
    def _badge(ok: bool, ok_text: str, warn_text: str) -> str:
        if ok:
            return f'<span style="color:#4ade80;font-weight:600">{escape(ok_text)}</span>'
        return f'<span style="color:#f87171;font-weight:600">{escape(warn_text)}</span>'

    rows = ""

    if analysis_unavailable:
        dropped = analysis_input.get("undecodable_rows", 0)
        rows += (
    ...
    <table>
      <thead><tr><th style="width:180px">Check</th><th>Status</th></tr></thead>
      <tbody>{rows}</tbody>
    </table>"""
    ...
    all_imb = anomalies.get("imbalanced_gpus", [])
    imb = [i for i in all_imb if not i.get("low_confidence")]
    skipped = anomalies.get(
        "imbalance_exclusions",
        [i for i in all_imb if i.get("low_confidence")],
    )
    comparison_status = anomalies.get("imbalance_comparison_status")
    if comparison_status is None:
        eligible = max(0, len(per_gpu) - len(skipped))
...
<body>
  ...
  <table>
    <thead><tr><th>Metric</th><th>Min</th><th>Max</th><th>Avg</th><th>Samples</th></tr></thead>
