...
def _has_finite_telemetry(row: dict) -> bool:
    for metric in _DYNAMIC_TELEMETRY_FIELDS:
        if _finite_field(row, metric) is not None:
            return True
    return False
...
class MonitoringEvidence:
    ...
    coverage_start_timestamp: Optional[int] = None
    coverage_end_timestamp: Optional[int] = None
    ...
    def worst_coverage(self, duration_sec: int) -> Tuple[int, Optional[str]]:
    ...
    @property
    def worst_covered_gpu(self) -> Optional[str]:
...
def _finite_field(row: dict, field_name: str) -> Optional[float]:
...
class Monitor:
    ...
    def __init__(self, csv_file: Path, cu_csv: Path, sample_interval: int = 1,
        ...
        self._monitor_bg: Optional["AbstractBackgroundProcess"] = None
        ...
        self._cu_sample_proc: Optional[subprocess.Popen] = None
    ...
    def _start_monitor_proc(self) -> None:
        ...
        self._monitor_proc = subprocess.Popen(
            [self._amd_smi, "monitor", "-p", "-t", "-u", "-m", "-v",
             "-w", str(self.sample_interval), "--csv", "--overwrite",
    ...
    def __enter__(self) -> "Monitor":
        ...
        try:
            ...
            if rc is not None:
                print(f"  [monitor] WARNING: amd-smi monitor exited immediately "
    ...
    def _report_monitor_death(self) -> None:
        ...
        try:
            ...
            pass
        rc = self._monitor_poll()
        print(f"  [monitor] WARNING: amd-smi monitor exited on its own after "
    ...
    *,
    ...
    if sample_times:
        ...
        for gpu, times in sorted(sample_times.items()):
            ...
            gap = max(
                (b - a for a, b in zip(ordered, ordered[1:])), default=0,
            )
