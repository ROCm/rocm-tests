...
RCQT runs unprivileged and ``rcqt_single`` maps to ``./`` in the config mapping,
so it is device-agnostic.
...
On a deb/rpm host those counters are the verdict: an action passes only when
nothing is missing or version-mismatched.

On a TheRock install there are no package-manager records to check, and the RVS
summary table cannot stand in for them -- RVS prints ``PASS`` there even when
every package is missing, as observed on this hardware (0 installed, 35 missing,
summary ``PASS``). Deciding on that table alone cannot fail for any reason short
of RVS itself printing FAIL, so this test skips instead: if ROCm was not
installed by the package manager there is genuinely nothing for RCQT to qualify,
and saying so is more honest than reporting a pass.
...
def _parse_rcqt_actions(text: str) -> dict[str, dict]:
    ...
    for line in text.splitlines():
        ...
        if match := _INSTALLED_RE.search(line):
            ...
            # Counted per action; a single counter shared across actions would
            # leak one action's failures into the next.
