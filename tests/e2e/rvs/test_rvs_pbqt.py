...
PBQT runs unprivileged and needs at least two GPUs, so a single-GPU host is
skipped as UNSUPPORTED. For each ordered GPU pair it reports peer reachability
and then, where the action enables it, measured bandwidth::
...
* Peer state and bandwidth validity are checked as separate, meaningful
  conditions. A single predicate such as ``'true' in peers_value or
  "peers:false" in peers_value`` is satisfied by both ``peers:true`` and
  ``peers:false``, so it always holds and reduces the test to a sample count.
* Errors fail the run, via the shared log check rather than a module-local
  pattern, so an ``RVS-ERROR`` line cannot pass through silently.
