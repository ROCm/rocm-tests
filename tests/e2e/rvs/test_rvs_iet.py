...
IET runs unprivileged. It ramps a GEMM workload towards ``target_power`` and
reports one verdict per (action, GPU) alongside a stream of power samples::
...
* No Multi-Chip Module special case is applied. Keying verdicts by
  ``(action, GPU)`` and requiring each to be ``TRUE`` needs no die-count
  correction and is right on both single-die and MCM parts. Scaling an expected
  GPU count by the number of dies instead invites arithmetic that cannot hold
  on a fully passing host.
