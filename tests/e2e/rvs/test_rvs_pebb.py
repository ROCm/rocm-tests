...
PEBB runs unprivileged. It walks each CPU/GPU combination and reports the
measured host-to-device and device-to-host throughput::
...
* Direction flags are not read as verdicts. Passing an action whenever at least
  one direction is enabled qualifies a property of the config file, true before
  the test ran. Bandwidth values are validated instead.
* Parsing errors are not swallowed. A bare ``except:`` around the verdict loop
  would turn a parsing failure into an empty result set rather than a failure.
