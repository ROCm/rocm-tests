...
Unlike PEQT, GPUP runs unprivileged and emits *no* ``true``/``false`` verdict of
its own -- it only prints the property values it read:
...
* The node for a GPU is taken from ``rvs -g``, which reports
  ``GPU[ <node> - <gpu_id> ]``. Listing order cannot be assumed to match
  sequential node indices: on an 8x MI210 host ``rvs -g`` is ordered by PCI
  address and maps to nodes 4,5,3,2,8,9,7,6, so deriving a node from its
  position in the listing pairs properties with the wrong GPU.
* Properties are compared per node and per io_link. Accumulating every node's
  properties into one list shared by all GPUs would weaken the check to
  "appears on *some* GPU" and would mask a mapping error of the kind above.
