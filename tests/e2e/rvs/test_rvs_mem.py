...
MEM runs unprivileged. It walks a set of numbered memory tests (walking-1-bit,
own-address, moving inversions, block move, random patterns) on each GPU in
parallel and reports one verdict per test per GPU::
...
* ``FAIL`` verdicts are matched explicitly. Scanning only for ``: PASS`` plus a
  final-status sweep for ``FALSE``/``ABORT``/``ERROR`` would miss a
  ``mem Test 4 : FAIL`` line entirely and leave a real memory failure green.
* Verdicts are read for whichever actions the config declares, rather than a
  hard-coded ``[action_1]``, which would make every further action invisible.
* Per-GPU coverage is enforced, so collapsing all GPUs into a single key per
  test number cannot hide how many GPUs actually reported in.
