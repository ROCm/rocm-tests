...
This is the only RVS test that runs no GPU workload and needs no config file: it
just asks ``rvs -g`` what RVS can see and compares that against the SMI's own
inventory::
...
Two properties of the comparison matter:

* It is symmetric. Checking only ``rvs_gpu_set.difference(smi_gpu_set)`` passes
  whenever RVS enumerates a *subset* of the GPUs the SMI reports, so an RVS
  build that saw three of eight GPUs would qualify as PASS -- exactly the
  regression this test exists to catch. Both directions are checked and
...
* Node ids and BDFs are compared, not just the id set. ``amd-smi list`` reports
  ``NODE_ID`` directly, so the stronger check is available: a GPU present in
  both views but attributed to the wrong node would slip past an id-only
  comparison.
...
def test_rvs_list_gpus(target_executor, rvs_binary, rvs_env, rock_dir):
    ...
    # Guards against a silent regression to a KFD-id-only comparison on a host
    # where amd-smi does supply the richer fields.
