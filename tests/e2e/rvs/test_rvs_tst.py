...
TST runs unprivileged. It ramps a GEMM workload towards ``target_temp`` and
reports one verdict per (action, GPU) alongside a stream of temperature
samples::
...
TRUE`` while never reading a usable temperature, so every reported temperature
must also be greater than zero. That check carries a gfx942 exemption -- MI300
legitimately reports 0 for these sensors.

How the verdict is built:

* Verdicts are keyed by ``(action, gpu)``. Storing them under the action name
  alone would overwrite seven of eight results on an 8-GPU host and leave only
  the last GPU's verdict standing.
* The reported ``TRUE``/``FALSE`` is compared as a value, not tested for
  truthiness. ``if status and flag`` is always true for a non-empty status
  string, which records a genuine ``pass: FALSE`` as a pass and leaves the run
  depending on a raw substring scan of the log to fail it.
"""
