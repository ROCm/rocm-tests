...
BABEL runs unprivileged. Unlike the other RVS modules it emits no per-GPU
``pass:`` verdict; it launches the BabelStream kernels and prints one throughput
table covering every GPU::
...
* Every action is reported whether it passed or failed, so a failure in one
  action cannot hide the results of the others.
* An action that produced no table at all fails. An empty failure list is not
  on its own a pass, because it also covers the case where no rows were parsed.
* The expected kernel set is read from the config's ``copy``/``mul``/``add``/
  ``triad``/``dot`` flags rather than hard-coded to all five, so a config that
  disables a kernel is not failed for the missing row.
