...
# ---------------------------------------------------------------------------
# Helper Functions (nested Docker, test layout detection)
# ---------------------------------------------------------------------------

_DEFAULT_DOCKER_SOCK = "/var/run/docker.sock"


def _vllm_nested_docker_unconfigured():
    """True when docker_callf cannot use a local Unix socket and no TCP override is set."""
    if not os.path.isfile("/.dockerenv"):
        return False
    if os.environ.get("ROCMTEST_DOCKER_HOST", "").strip():
        return False
    docker_host = os.environ.get("DOCKER_HOST", "").strip()
    if docker_host.startswith(("tcp://", "http://", "https://")):
        return False
    sock = _DEFAULT_DOCKER_SOCK
    if docker_host.startswith("unix://"):
        sock = docker_host[len("unix://") :] or _DEFAULT_DOCKER_SOCK
    elif docker_host:
        return False
    return not os.path.exists(sock)


def _vllm_tests_layout_present():
    """True if vLLM pytest tree exists here (same paths as run_unit_tests)."""
    home_tests = os.path.join(os.path.expanduser("/app"), "vllm", "tests")
    ws_tests = os.path.join(os.path.expanduser("/vllm-workspace"), "tests")
    return os.path.isdir(home_tests) or os.path.isdir(ws_tests)
...
def _build_suite_command(pytest_target: str, hf_token: str, testcase: str = "") -> str:
    ...
    curl/libsodium23, export the multiproc method and HF token, pre-download required
    HuggingFace models, locate the vLLM tests tree, then run ``pytest -v -s <target>``.
    The final exit code is pytest's, so ``result.ok`` reflects suite pass/fail.

    Model downloads are based on the testcase to match upstream requirements.
    """
...
def _run_vllm_suite(vllm_container, hf_token: str, testcase: str, label: str) -> None:
    ...
    command = _build_suite_command(target, hf_token, testcase)
