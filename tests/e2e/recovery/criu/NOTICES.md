...
build directory (cuda_memtest, pytorch/examples, Kokkos). (CRIU, the checkpoint/restore tool
...
- **Modification imposes no obligation.** NCSA, BSD-3-Clause, and Apache-2.0 are permissive:
  ...
  only on *redistribution*, which does not occur. pytorch/examples and Kokkos are not modified.
- **If distribution is ever added** (e.g. bundling the sources or built binaries): retain the NCSA
  ...
  pytorch/examples; and retain the Apache-2.0 license, `NOTICE` file, and attribution for Kokkos.
  This is out of scope for the current runtime-fetch model.
