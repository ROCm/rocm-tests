...
def _check_headers(rock_dir: str) -> None:
    ...
    if not kernel_lib.exists() or not any(kernel_lib.glob("*.dat")) and not any(kernel_lib.glob("*.so*")):
        pytest.skip(
            f"hipblaslt kernel libraries not found under {rock_dir}/lib/hipblaslt/library/. "
            "Install or rebuild ROCm with hipBLASLt: apt install rocm-hipblaslt-dev"
        )
