...
GPU architecture gate:
    hipBLASLt provides no FP8 GEMM kernels on pre-CDNA3 datacenter parts or on
    RDNA up to 3.5, so the heuristic finds no algorithm there. Those families are
...
# Listed as unsupported prefixes rather than an allowlist so a new architecture
# runs by default instead of being silently skipped.
_NO_FP8_ARCH_PREFIXES = ("gfx90", "gfx100", "gfx101", "gfx103", "gfx110", "gfx115")


@pytest.mark.runtime.fast
@pytest.mark.parametrize("check", _CHECKS)
def test_hipblaslt_zero_mat(
    target_executor, ld_path: dict, hipblaslt_zero_mat_binary: str, gpu_arch: str | None, check: str
):
    """FP8/BF8 GEMM with one zero operand yields an all-zero output."""
    if gpu_arch and gpu_arch.startswith(_NO_FP8_ARCH_PREFIXES):
        pytest.skip(f"hipBLASLt has no FP8 GEMM kernels on {gpu_arch}; FP8 zero-matrix check does not apply")
