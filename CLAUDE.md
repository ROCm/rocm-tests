...
`rocm-test` is an AMD ROCm System End-to-End Test Framework. It validates the full ROCm software stack (kernel driver → HIP runtime → compute libraries → ML frameworks) on AMD GPU hardware. 
...
pylint framework --fail-under=8.0
```
...
| `runtime.*` | YES (always explicit) | `fast` (<5 min), `medium` (<30 min), `longevity` (<2 hr), `soak` (hours) |
...
| `--gpu-acquire-timeout N` | 30 s | `remote_node_plugin` |
...
build_timeout_secs = 7200.0
build_inactivity_timeout_secs = 600.0
...
[notifications]
webhook_url = ""           # set via ROCM_TEST_NOTIFICATIONS_WEBHOOK_URL only
notify_on = ["FAIL", "REGRESSION", "HEALTH_FAIL"]
```
...
|---|---|---|
| `pre-commit.yml` | Every PR | No |
| `e2e-nightly.yml` | UTC daily, cron | Yes (`gfx942`/`gfx1100`) |
| `e2e-weekly.yml` | UTC extended weekly, cron | Yes |
| `pr-validation.yml` | Every PR to `main` | No (`--no-gpu`) |
| `security-scan.yml` | Scheduled | No |
