...
The updater is intended to run from the e2e-nightly upload job after all
per-platform jobs have uploaded their ``test-counts-*`` artifacts. It inserts
one row per platform plus one computed ``TOTAL`` row.

All connection parameters (host, port, username, password, database, table)
must be supplied via environment variables or CLI flags — no values are
hardcoded in this module.

Required environment variables::

    CLICKHOUSE_HOST      ClickHouse server hostname or IP
    CLICKHOUSE_PORT      Native HTTPS port (default 8443)
...
Optional environment variables::
    ...
    CLICKHOUSE_VERIFY_SSL     Set to "false" to disable TLS verification
    ...
    GITHUB_TOKEN          GitHub personal-access token for the Jobs API
    GITHUB_VERIFY_SSL     Set to "false" to disable GitHub API TLS verification

Examples:
    Typical nightly CI invocation (all secrets injected by GitHub Actions)::
        ...
        python -m framework.results.backends.clickhouse \\
            ...
            --table rocmtests_nightly_runs \\
            --testplan testplan.ini \\
            --counts-dir all-counts

    Create the destination table on first use, then insert::
    ...
    Insert without appending the computed TOTAL row::
...
def ssl_context() -> ssl.SSLContext | None:
    ...
    When ``GITHUB_VERIFY_SSL`` is falsy, TLS verification is disabled
    explicitly via the public ``ssl`` API (not the private
    ``ssl._create_unverified_context``).  This should only be used in
    controlled test environments — never in production.
    """
...
def fetch_json(url: str) -> dict[str, Any]:
    ...
    ) as response:  # nosec B310 — URL is always https://api.github.com/; never derived from user input
...
def build_insert_rows(
    *,
    ...
    if include_total_row and rows:
        total_pass = sum(row[column_index["tests_pass"]] for row in rows)
        total_fail = sum(row[column_index["tests_fail"]] for row in rows)
        total_error = sum(row[column_index["tests_error"]] for row in rows)
        total_skip = sum(row[column_index["tests_skip"]] for row in rows)
        total_count = sum(row[column_index["total_test_count"]] for row in rows)
        ...
        def set_total_value(column: str, value: Any) -> None:
            total_row[column_index[column]] = value
